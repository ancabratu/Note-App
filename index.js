...
const createNoteView = (note) => {
  const noteDive = createElement("div", ["note"]);
  noteDive.id = note.id;
  ...
  editButton.innerHTML = createElement("button", ["delete"]);
...
const editNote = (noteDiv, editSave = false) => {
  ...
  deleteButton.innerHTML = "Cncel Edit";
...
const saveNote = () => {
  ...
  // add user to reset the data and add a new one
  titleInput.value = "";
  bodyInput.value = "";
  bgColorInput.value = "";
};
